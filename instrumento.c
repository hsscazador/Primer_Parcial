...
* \param len int
* \param id int
* \return Return Instrumento index position or (-1) if [Invalid length or NULL
pointer received or Instrumento not found]
*
*/
...
/** \brief Remove a Instrumento by Id (put isEmpty Flag in 1)
*
* \param list Instrumento*
* \param len int
* \param id int
* \return int Return (-1) if Error [Invalid length or NULL pointer or if can't
find a Instrumento] - (0) if Ok
*
*/
int removeInstrumento(Instrumento* list, int len, int id)
{
    ...
    index=findInstrumentoId(list, len, id); //Busca ID y regresa indice.
    if(index!=-1&&list!=NULL)
    {
        ...
        printf("|%6d|%51s|%51s|\n",list[index].id, list[index].name, list[index].Tipo);
...
int printInstrumento(Instrumento* list, int length)
{
    ...
    for(i=0;i<length;i++)
    {
        if(list[i].isEmpty==0)
        {
            printf("|%6d|%51s|%51s|\n",list[i].id,list[i].name,list[i].Tipo);
        }
    }
    barra(72);
